...
class Auth {
	...
	public static function login_and_get_token( $username, $password ) {
		...
		/** */
		$results = apply_filters( 'graphql_jwt_auth_before_authenticate', $username, $password );

		if ( $results['status'] === 'fail' ) :
			throw new UserError( __( $results['message']['label'], 'wp-graphql-jwt-authentication' ) );
		endif;
